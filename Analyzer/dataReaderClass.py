...
class DataReader:
        ...
        for dir in self.directories:
            ...
            for n in range(1, 51):  # TODO: This 50 should be deduced from .nml file!
                psiPath = os.path.join(
                    self.runsPath, dir, "OutputData", f"Psi_1_n{n}.dat"
                )
    ...
    def LoadSingleElectronEnergies(self):
        for dir in self.directories:
            # print(dir)
            energiesPath = os.path.join(
                self.runsPath, dir, "OutputData", f"Energies1.dat"
            )
            if os.path.exists(energiesPath):
                energies = pd.read_fwf(
                    energiesPath, skiprows=1, infer_nrows=100, names=["state", "E"]
                )
    ...
    def LoadSingleElectronExpectations(self):
        for dir in self.directories:
            # print(dir)
            expectationsPath = os.path.join(
                self.runsPath, dir, "OutputData", f"Expectations_1.dat"
            )
            if os.path.exists(expectationsPath):
                expectations = pd.read_fwf(
                    expectationsPath,
                    skiprows=1,
                    infer_nrows=100,
                    names=[
    ...
    def LoadMultiElectronEnergies(self):
        for dir in self.directories:
            # print(dir)
            energiesPath = os.path.join(
                self.runsPath, dir, "OutputData", f"Energies2.dat"
            )
            if os.path.exists(energiesPath):
                energies = pd.read_fwf(
                    energiesPath, skiprows=1, infer_nrows=100, names=["state", "E"]
    ...
    def LoadMultiElectronExpectations(self):
        for dir in self.directories:
            print(dir)
            expectationsPath = os.path.join(
                self.runsPath, dir, "OutputData", f"Expectations_2.dat"
            )
            if os.path.exists(expectationsPath):
                expectations = pd.read_fwf(
                    expectationsPath,
    ...
    def LoadTimeDependence(self, dir):
        ...
        timeDependencePath = os.path.join(
            self.runsPath, dir, "OutputData", f"Time_dependent.dat"
        )
        if os.path.exists(timeDependencePath):
            timeDependence = pd.read_fwf(
                timeDependencePath, skiprows=0, infer_nrows=100, names=header
            )
