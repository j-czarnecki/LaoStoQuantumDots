...
class Plotter(DataReader):
    ...
    def PlotSingleElectronEnergies(self, colorParam: str):
        ...
        for state in range(0, 50):
            ...
            lc = LineCollection(
                segments, array=interp_color, cmap="coolwarm", norm=plt.Normalize(-1, 1)
            )
            lc.set_linewidth(2)
    ...
    def PlotMultiElectronEnergies(self, colorParam: str):
        spins_cmap = LinearSegmentedColormap.from_list(
            "spins_cmap", ["deepskyblue", "black", "deeppink"]
        )
        plt.figure()
        fig, ax = plt.subplots(figsize=(6, 10))
    ...
    def PlotTimeDependence(self):
        ...
        for i in range(1, 6):
            plt.plot(df["t"], df[f"c_{i}"], label=rf"$|c_{i}|^2$")
        plt.legend()
        plt.xlabel(r"$t$ (ns)")
        plt.ylabel(r"$|c_n|^2$")
